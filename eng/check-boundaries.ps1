...
foreach ($sourceFile in $sourceFiles) {
    ...
    if ($source -match 'Gma\.Modules\.(?!Auth(?:\.|;))') {
    ...
    if ($relativePath -match '^src\\Gma\.Modules\.Auth\.(?:Api|AdminApi|AdminCli)\\' -and
        $source -match 'Gma\.Modules\.Auth\.Domain') {
        $errors.Add("$relativePath crosses the front-door to domain boundary.")
    }
}

if ($errors.Count -gt 0) {
    throw "Auth boundary checks failed:`n - $($errors -join "`n - ")"
}

Write-Host 'Auth boundary checks passed.'
