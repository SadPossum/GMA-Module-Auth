namespace Gma.Modules.Auth.Persistence.Configurations;

using Gma.Modules.Auth.Domain.Aggregates;
using Gma.Modules.Auth.Domain.Entities;
using Gma.Modules.Auth.Domain.ValueObjects;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.ChangeTracking;
using Microsoft.EntityFrameworkCore.Metadata.Builders;

internal sealed class MemberAuthenticationChallengeConfiguration
    : IEntityTypeConfiguration<MemberAuthenticationChallenge>
{
    public void Configure(EntityTypeBuilder<MemberAuthenticationChallenge> builder)
    {
        builder.ToTable("member_authentication_challenges");
        builder.HasKey(challenge => challenge.Id);

        builder.Property(challenge => challenge.Id)
            .HasConversion(id => id.Value, value => new MemberAuthenticationChallengeId(value));

        builder.Property(challenge => challenge.MemberId)
            .HasConversion(id => id.Value, value => new MemberId(value));

        builder.Property(challenge => challenge.TokenHash)
            .HasMaxLength(MemberAuthenticationChallenge.TokenHashMaxLength)
            .IsRequired();

        builder.Property(challenge => challenge.PrimaryAuthenticationMethod)
            .HasMaxLength(MemberAuthenticationMethods.MaxLength)
            .IsRequired();

        builder.Property(challenge => challenge.PrimaryAuthenticationContextReference)
            .HasMaxLength(SessionAuthenticationEvidence.ContextReferenceMaxLength)
            .IsRequired();

        builder.Property<string[]>("primaryAuthenticationMethodReferences")
            .HasField("primaryAuthenticationMethodReferences")
            .HasColumnName("primary_authentication_method_references")
            .HasConversion(
                values => SessionAuthenticationMethodReferencesJson.Serialize(values),
                value => SessionAuthenticationMethodReferencesJson.Deserialize(value))
            .HasMaxLength(SessionAuthenticationMethodReferencesJson.MaxLength)
            .IsRequired()
            .Metadata.SetValueComparer(new ValueComparer<string[]>(
                (left, right) => left.SequenceEqual(right),
                values => values.Aggregate(0, (hash, value) => HashCode.Combine(hash, value)),
                values => values.ToArray()));

        builder.Property(challenge => challenge.IpAddress)
            .HasMaxLength(MemberAuthenticationChallenge.IpAddressMaxLength);
        builder.Property(challenge => challenge.UserAgent)
            .HasMaxLength(MemberAuthenticationChallenge.UserAgentMaxLength);
        builder.Property(challenge => challenge.PrimaryAuthenticatedAtUtc).IsRequired();
        builder.Property(challenge => challenge.CreatedAtUtc).IsRequired();
        builder.Property(challenge => challenge.ExpiresAtUtc).IsRequired();
        builder.Property(challenge => challenge.MaximumAttempts).IsRequired();
        builder.Property(challenge => challenge.FailedAttemptCount).IsRequired();
        builder.Property(challenge => challenge.ConcurrencyStamp)
            .IsConcurrencyToken()
            .IsRequired();

        builder.HasOne<Member>()
            .WithMany()
            .HasForeignKey(challenge => challenge.MemberId)
            .HasPrincipalKey(member => member.Id)
            .OnDelete(DeleteBehavior.Cascade);

        builder.HasIndex(challenge => new { challenge.ScopeId, challenge.TokenHash }).IsUnique();
        builder.HasIndex(challenge => new { challenge.ScopeId, challenge.MemberId, challenge.CreatedAtUtc });
        builder.HasIndex(challenge => challenge.ExpiresAtUtc);
        builder.HasIndex(challenge => challenge.ConsumedAtUtc);
        builder.HasIndex(challenge => challenge.RevokedAtUtc);
    }
}
