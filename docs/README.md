...
Implementation planning: [Global Identity With Ambient Tenancy](global-identity-with-tenancy-task.md), [Trusted Subject Status Reading](auth-subject-status-reading-task.md), [Account Recovery](account-recovery-task.md), [Authentication Assurance And Step-Up](authentication-assurance-task.md), [TOTP Authenticator Lifecycle And Recovery](mfa-authenticator-lifecycle-task.md), [Auth Production Hardening](auth-production-hardening-task.md), and [Auth Domain Completion Audit](auth-domain-completion-audit-task.md).
...
Trusted orchestration hosts may opt into `IAuthSubjectStatusReader` with
`builder.AddAuthSubjectStatusReader()`. It resolves one opaque subject in the
currently composed fixed or ambient Auth scope and returns only persisted scope,
canonical subject, and current member status. It is not registered by normal
Auth composition and must not be exposed as a public account-discovery endpoint.
Consumers decide what an active account may own and must use the canonical
returned subject.
